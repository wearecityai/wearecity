...
import TravelExploreIcon from '@mui/icons-material/TravelExplore'; // For "Descubrir Gems"
import HistoryIcon from '@mui/icons-material/History'; // For "Actividad"
import SettingsIcon from '@mui/icons-material/Settings'; // For "Ajustes y Ayuda"
...
const App = ({ toggleTheme, currentThemeMode }: AppProps): JSX.Element => {
  ...
  const drawerWidth = 260; // Adjusted drawer width
  ...
  const buildFullSystemInstruction = useCallback((config: CustomChatConfig, location: UserLocation | null): string => {
    ...
    if (config.uploadedProcedureDocuments && config.uploadedProcedureDocuments.length > 0) {
        ...
        systemInstructionParts.push(UPLOADED_PDF_PROCEDURE_INSTRUCTION_TEMPLATE); // This is a template string, should be fine if it's used correctly later.
    }
  ...
  }, []); // Added empty dependency array for useCallback, as it uses constants or args
  ...
  }, [isGeminiReady, chatConfig, userLocation, messages, initializeChatAndGreet]);
  ...
  const drawerContent = (
    <Box sx={{ width: drawerWidth, height: '100%', display: 'flex', flexDirection: 'column', bgcolor: 'background.paper', pt: 1 }} role="presentation">
      <Button
        variant="contained"
        startIcon={<EditOutlinedIcon />}
        onClick={() => handleNewChat("Nuevo chat")}
        sx={{
          m: '12px 16px', // Matches Gemini's button margin
          bgcolor: theme.palette.mode === 'dark' ? '#2e2f32' :'#e0e0e0', // custom dark/light color for this button
      ...
      >
       ...
       <List sx={{ flexGrow: 1, px:1 }}>
         <ListItemButton onClick={() => console.log("Descubrir Gems clicked")}> {/* Placeholder */}
            <ListItemIcon sx={{minWidth: 36}}><TravelExploreIcon /></ListItemIcon>
            <ListItemText primary="Descubrir Gems" primaryTypographyProps={{fontSize: '0.875rem'}} />
          </ListItemButton>

        <Typography variant="caption" sx={{ px: 2, py: 1, color: 'text.secondary', fontWeight: 500, display:'block' }}>
          RECIENTE
        </Typography>
        {chatTitles.map((title, index) => (
          <ListItemButton
            ...
            onClick={() => {
                ...
                // In a real app, load this chat's messages
         ...
         <ListItemButton onClick={() => console.log("Mostrar más clicked")}> {/* Placeholder */}
            <ListItemIcon sx={{minWidth: 36}}><ExpandMoreIcon /></ListItemIcon>
      ...
      <List sx={{pb:1, px:1}}>
        <Divider sx={{my:1}}/>
        <ListItemButton onClick={() => console.log("Actividad clicked")}> {/* Placeholder */}
            <ListItemIcon sx={{minWidth: 36}}><HistoryIcon /></ListItemIcon>
            <ListItemText primary="Actividad" primaryTypographyProps={{fontSize: '0.875rem'}}/>
        </ListItemButton>
        <ListItemButton onClick={() => { setCurrentView('finetuning'); if (isMobile) setIsMenuOpen(false); }}>
            <ListItemIcon sx={{minWidth: 36}}><SettingsIcon /></ListItemIcon>
            <ListItemText primary="Ajustes y ayuda" primaryTypographyProps={{fontSize: '0.875rem'}}/>
        </ListItemButton>
        <Divider sx={{my:1}}/>
        <ListItem sx={{display: 'flex', flexDirection:'column', alignItems:'flex-start', pt:0.5, pb:0.5}}>
            ...
            <Button
                variant="text"
                size="small"
                onClick={() => chatConfig.allowGeolocation && navigator.geolocation.getCurrentPosition(() => {}, () => {}, {})} /* Trigger re-fetch or open settings */
  ...
  return (
    <Box sx={{ display: 'flex', height: '100vh', maxHeight: '100vh', overflow: 'hidden', bgcolor: 'background.default' }}>
      ...
      <Box component="main" sx={{
        ...
        alignItems: 'stretch', // Ensure children (AppBar, Stack) stretch horizontally
        height: '100vh',
        maxHeight: '100vh',
        width: `calc(100% - ${isMobile || !isMenuOpen ? 0 : drawerWidth}px)`,
        marginLeft: isMobile || !isMenuOpen ? 0 : `${drawerWidth}px`,
        ...
        <AppBar 
            ...
            sx={{ 
                ...
                width: '100%' // Ensure AppBar takes full width of its allocated space
            }}
        >
          <Toolbar sx={{ minHeight: '56px!important', px: {xs: 1, sm: 2} }}>
            ...
            <Button
                ...
                // onClick={handleModelSelectClick} // Placeholder
            >
                2.5 Flash
            </Button>
        ...
        <Stack
            ...
            sx={{
                ...
                overflow: 'hidden',
                position: 'relative',
                width: '100%', // Stack takes full width of its stretched allocation from MainContentBox
                // Conditional maxWidth and mx (horizontal margin) determine if Stack itself is centered or full-width
                maxWidth: (isMenuOpen && !isMobile) || isMobile ? '100%' : '800px',
                mx: (isMenuOpen && !isMobile) || isMobile ? 0 : 'auto',
            }}
        >
            ...
            <Alert
                ...
                sx={{ mx: {xs: 1, sm:2}, mt:1, borderRadius: 2 }} 
                iconMapping={{
