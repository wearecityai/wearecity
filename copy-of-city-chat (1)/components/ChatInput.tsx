...
import { Box, TextField, IconButton, Paper, Stack, Chip, Menu, MenuItem, Button, Typography, CircularProgress, useTheme, ListItemIcon, ListItemText } from '@mui/material';
import SendIcon from '@mui/icons-material/Send';
import MicIcon from '@mui/icons-material/Mic';
import MicOffIcon from '@mui/icons-material/MicOff';
import AddIcon from '@mui/icons-material/Add'; // For the "+" button
import CheckIcon from '@mui/icons-material/Check';
import AutoFixHighIcon from '@mui/icons-material/AutoFixHigh'; // For "Deep Research"
import BrushIcon from '@mui/icons-material/Brush'; // For "Canvas"
...
const ChatInput: React.FC<ChatInputProps> = ({
  ...
  return (
    <Box sx={{ p: { xs: 1, sm: '12px 20px' }, bgcolor: 'background.default', borderTop: `1px solid ${theme.palette.divider}` }}>
      {isRecording && (
        <canvas
          ref={canvasRef}
          style={{
            width: 'calc(100% - 40px)', // Adjust based on outer padding
      ...
      >
        ...
        <Stack direction="column" sx={{ flexGrow: 1, mx: 0.5 }}>
            {!isRecording && (
                 <Stack direction="row" spacing={1} sx={{ mb: inputValue ? 0 : 0.5, alignItems:'center', display: inputValue ? 'none' : 'flex' }}>
                    <Chip icon={<AutoFixHighIcon />} label="Deep Research" size="small" onClick={() => onSendMessage("Explain with Deep Research: ")} sx={{cursor: 'pointer'}} />
                    <Chip icon={<BrushIcon />} label="Canvas" size="small" onClick={() => onSendMessage("Generate image for Canvas: ")} sx={{cursor: 'pointer'}} />
                </Stack>
            )}
            <TextField
