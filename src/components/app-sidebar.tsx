      ...
      <Sidebar variant="sidebar" collapsible="icon" className="sidebar-transition" {...props}>
        ...
        <SidebarContent className="flex flex-col gap-0 h-full sidebar-content-transition">
          ...
          <SidebarGroup className="flex-1 flex flex-col min-h-0">
            ...
            {/* Conversations Title - First, above everything */}
            <div className="group-data-[collapsible=icon]:hidden flex h-8 shrink-0 items-center rounded-md px-2 text-xs font-medium text-sidebar-foreground/70">
              {t('sidebar.conversations', { defaultValue: 'Conversaciones' })}
            </div>
                ...
                <SidebarMenuButton
                  onClick={() => {
                    ...
                    if (isAdminContext) {
                      ...
                      const slug = chatConfig?.assistantName ? generateChatSlug(chatConfig.assistantName) : (chatConfig?.restrictedCity?.slug as string | undefined) || getCurrentCitySlug() || '';
