...
class RealTimeSearchService {
        ...
        try {
            ...
            if (placeType) {
                ...
                const response = await googlePlacesService_1.googlePlacesService.searchPlaces({
                    query: specificQuery,
                    location,
                    radius: userType === 'tourist' ? 2000 : 1500,
                    maxResults: 10,
    ...
    async searchByCategory(category, city, location) {
        ...
        try {
            switch (category) {
                case 'restaurants':
                    if (location) {
                        const places = await googlePlacesService_1.googlePlacesService.searchRestaurants(location, 1500, city);
                        const filteredPlaces = this.filterPlacesByCity(places, city);
                ...
                case 'monuments':
                    if (location) {
                        const places = await googlePlacesService_1.googlePlacesService.searchMonuments(location, 2000, city);
                        const filteredPlaces = this.filterPlacesByCity(places, city);
                ...
                case 'institutions':
                    if (location) {
                        const places = await googlePlacesService_1.googlePlacesService.searchInstitutions(location, 2000, city);
                        const filteredPlaces = this.filterPlacesByCity(places, city);
                ...
                case 'transport':
                    if (location) {
                        const places = await googlePlacesService_1.googlePlacesService.searchTransportStations(location, 1500, city);
                        const filteredPlaces = this.filterPlacesByCity(places, city);
                ...
                case 'emergency':
                    if (location) {
                        const hospitals = await googlePlacesService_1.googlePlacesService.searchHospitals(location, 2000, city);
                        const pharmacies = await googlePlacesService_1.googlePlacesService.searchPharmacies(location, 1500, city);
     ...
     */
    filterPlacesByCity(places, restrictedCity) {
        if (!restrictedCity || !places)
            return places;
        ...
        const filteredPlaces = places.filter(place => {
            ...
            if (restrictedPostalCodes.length > 0) {
                const hasValidPostalCode = restrictedPostalCodes.some(postalCode => fullAddress.includes(postalCode));
     ...
     */
    async enrichPlacesWithDetails(places, query) {
        const enrichedPlaces = await Promise.all(places.map(async (place) => {
            try {
                ...
                return googlePlacesService_1.googlePlacesService.convertToPlaceCardInfo(detailedPlace, query);
            }
            catch (error) {
                console.warn(`⚠️ Failed to get details for place ${place.name}:`, error);
                // Usar datos básicos si falla la obtención de detalles
                return googlePlacesService_1.googlePlacesService.convertToPlaceCardInfo(place, query);
            }
        }));
