...
exports.configureGenericInstructions = exports.getCityInstructions = exports.configureCityInstructions = exports.chatIA = void 0;
const functions = __importStar(require("firebase-functions"));
const admin = __importStar(require("firebase-admin"));
const aiplatform_1 = require("@google-cloud/aiplatform");
const realTimeSearchService_1 = require("./realTimeSearchService");
...
exports.chatIA = functions.https.onCall({
    memory: '512MiB',
    timeoutSeconds: 60
}, async (data, context) => {
    try {
        const { userMessage, userId, userLocation, allowMapDisplay = false, customSystemInstruction = '', citySlug, cityId, conversationHistory = [], mode = 'quality', historyWindow = 10, userContext = {} } = data;
...
🤖 **ASISTENTE**:`;
        ...
        const response = result[0].predictions?.[0]?.candidates?.[0]?.content?.parts?.[0]?.text || 'Error generando respuesta';
 ...
 */
async function getSystemPromptFromFirebase(cityIdentifier, customInstruction, userContext) {
    try {
        let instructions = '';
        if (cityIdentifier) {
 ...
 */
async function getCityConfigFromFirebase(cityIdentifier) {
    try {
 ...
 */
async function getDefaultCityConfigFromFirebase(cityName) {
    try {
        const db = admin.firestore();
 ...
 */
async function getGenericInstructionsFromFirebase() {
    try {
        const db = admin.firestore();
 ...
 */
function getFallbackGenericInstructions() {
 ...
 */
function generateInstructionsFromFirebase(config) {
...
async function processResponse(aiResponse, userMessage, citySlug, userContext, realTimeData) {
    ...
    if (realTimeData) {
        ...
        if (realTimeData.places && realTimeData.places.length > 0) {
            ...
            realTimeData.places.forEach((place, index) => {
                response += `${index + 1}. **${place.name}**\n`;
                response += `   📍 ${place.address}\n`;
                if (place.rating)
                    response += `   ⭐ ${place.rating}/5\n`;
                if (place.types)
                    response += `   🏷️ ${place.types.join(', ')}\n`;
                response += `\n`;
            });
        }
...
async function saveConversation(userId, cityIdentifier, userMessage, aiResponse, userContext) {
    const db = admin.firestore();
 ...
 */
exports.configureCityInstructions = functions.https.onCall(async (data, context) => {
 ...
 */
exports.getCityInstructions = functions.https.onCall(async (data, context) => {
    // Verificar autenticación
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Usuario debe estar autenticado');
    }
 ...
 */
exports.configureGenericInstructions = functions.https.onCall(async (data, context) => {
    // Verificar autenticación
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Usuario debe estar autenticado');
    }
 ...
 */
function shouldPerformRealTimeSearch(userMessage, userContext) {
 ...
 */
async function performIntelligentSearch(userMessage, citySlug, userLocation, userContext) {
    try {
        console.log('🔍 Starting intelligent search for:', userMessage);
        // Verificar disponibilidad del servicio
        if (!realTimeSearchService_1.realTimeSearchService.isAvailable()) {
        ...
        const searchContext = {
            ...
            userType: (userContext?.isTourist ? 'tourist' : 'resident'),
            interests: userContext?.interests || [],
            urgency: (userContext?.urgency || 'low')
        };
        const searchResults = await realTimeSearchService_1.realTimeSearchService.intelligentSearch(userMessage, searchContext);
