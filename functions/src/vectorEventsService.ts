...
export class VectorEventsService {
    ...
    try {
      ...
      try {
        ...
        if (vectorResult.data && vectorResult.data.length > 0) {
          ...
          const events = vectorResult.data.map((doc: any) => doc.eventCard || {
            title: doc.title,
            date: doc.date,
   ...
   */
  private async enhancedTraditionalSearch(
    query: string,
    realCityId: string,
    cityName: string,
    limit: number
    ...
    try {
      ...
      const scoredEvents = events.map(event => {
        ...
        if (daysFromNow <= 7) score += 2; // Eventos esta semana
        if (daysFromNow <= 3) score += 1; // Eventos próximos
   ...
   */
  private generateVectorResponse(events: EventCard[], cityName: string, query: string): string {
    if (events.length === 0) {
