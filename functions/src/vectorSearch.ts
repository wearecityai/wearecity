 ...
 */
export const vectorSearch = functions.https.onCall(async (data: VectorSearchRequest, context): Promise<VectorSearchResponse> => {
 ...
 */
export const hybridSearch = functions.https.onCall(async (data: any, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
  ...
  try {
    ...
    const vectorResults = await vectorSearch({ query, userId, citySlug, limit: limit * 2 }, context);
    ...
    const textMatches = textResults.docs
      .map(doc => {
        ...
        return {
          ...
          similarity: score / queryWords.length, // Normalizar por número de palabras
