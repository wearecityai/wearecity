 ...
 */
function extractDocumentLinks(links: any[]): string[] {
 ...
 */
function processContent(content: string, url: string): { title: string; cleanContent: string } {
 ...
 */
export const advancedScraping = functions.https.onCall(async (data: ScrapingRequest, context): Promise<ScrapingResult> => {
  ...
  try {
    ...
    const docRef = await db.collection('library_sources_enhanced').add({
      ...
      metadata: {
        ...
        language: 'es', // Detectar idioma después
        tags: [], // Generar tags después
 ...
 */
export const advancedCrawling = functions.https.onCall(async (data: any, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
