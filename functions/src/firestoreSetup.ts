 ...
 */
export const setupRAGCollections = async () => {
  ...
  try {
    ...
    await exampleSourceRef.set({
      ...
      embedding: null, // Se generará cuando se procese
      metadata: {
        wordCount: 12,
        language: 'es',
 ...
 */
export const setupRAGSystem = functions.https.onRequest(async (req, res) => {
  corsHandler(req, res, async () => {
    try {
      console.log('🚀 Setting up RAG system...');
      
      const result = await setupRAGCollections();
