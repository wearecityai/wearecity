 ...
 */
export const getSystemHealth = functions.https.onRequest(async (req, res) => {
  // Configurar CORS
  ...
  try {
    ...
    try {
      const puppeteerResponse = await fetch(
        'https://wearecity-puppeteer-service-294062779330.us-central1.run.app/health',
 ...
 */
export const getSystemMetrics = functions.https.onRequest(async (req, res) => {
  // Configurar CORS
  res.set('Access-Control-Allow-Origin', '*');
  res.set('Access-Control-Allow-Methods', 'GET, POST, OPTIONS');
  res.set('Access-Control-Allow-Headers', 'Content-Type, Authorization');
  ...
  try {
    ...
    res.json({
      ...
      period,
      stats,
      metrics: metrics.slice(0, 50), // Limitar a los últimos 50 registros
      timestamp: new Date().toISOString()
    });
