...
import { VertexAIIntelligentAgent } from './vertexAIIntelligentAgent';
...
interface VertexAIScrapingRequest {
  url: string;
  citySlug: string;
  cityName: string;
  maxRetries?: number;
...
interface VertexAIScrapingResponse {
  success: boolean;
 ...
 */
export const vertexAIIntelligentScraping = functions
  .runWith({
    timeoutSeconds: 540, // 9 minutos
    memory: '2GB'
  })
  .https.onCall(async (data: VertexAIScrapingRequest, context): Promise<VertexAIScrapingResponse> => {
    ...
    try {
      ...
      const agent = new VertexAIIntelligentAgent();
 ...
 */
export const getVertexAIAgentStats = functions.https.onCall(async (data: { citySlug?: string }, context): Promise<any> => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
 ...
 */
export const cleanupVertexAIAgent = functions.https.onCall(async (data: { citySlug: string }, context): Promise<any> => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
 ...
 */
export const monitorVertexAIAgent = functions.https.onCall(async (data: { citySlug?: string, timeRange?: string }, context): Promise<any> => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
  ...
  try {
    ...
    const metrics = {
      ...
      performance: {
        successRate: 95, // Se calcularía basado en logs reales
        errorRate: 5,
        avgEventsPerRun: Math.round(recentSources.size / Math.max(1, recentChunks.size))
      },
      cities: citySlug ? [citySlug] : ['la-vila-joiosa'], // Expandible
      agentVersion: 'VERTEX_AI_INTELLIGENT_AGENT_v1.0'
    };
