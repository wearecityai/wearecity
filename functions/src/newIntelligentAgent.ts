...
export class NewIntelligentAgent {
   ...
   */
  async extractEventsFromHTML(html: string, url: string): Promise<ScrapedEvent[]> {
    try {
...
`;
   ...
   */
  async scrapeIntelligently(url: string, maxRetries: number = 3): Promise<ScrapedEvent[]> {
    try {
   ...
   */
  async generateEventEmbeddings(events: ScrapedEvent[], citySlug: string): Promise<void> {
    try {
...
Contenido completo: ${event.fullContent}
   ...
   */
  async cleanupOldEvents(citySlug: string): Promise<{ eventsDeleted: number, chunksDeleted: number }> {
    try {
